...
def configured_element_manager() -> ElementManager:
    ...
    arg_map2 = {"args": ["forceBeamColumn", 2, 2, 3, 1, 1], "kwargs": {}}
...
def test_element_command_parsing(element_manager: ElementManager):
    ...
    assert element["eleType"] == "Truss"
    assert element["eleTag"] == 1
    assert element["eleNodes"] == [1, 2]
    assert element["A"] == 100
    assert element["matTag"] == 1
...
def test_beam_element_parsing(element_manager: ElementManager):
    ...
    assert element is not None
    assert element["eleType"] == "forceBeamColumn"
    assert element["eleTag"] == 2
    assert element["eleNodes"] == [1, 2]
    assert element["transfTag"] == 1
    assert element["integrationTag"] == 1
...
def test_get_elements_by_type(configured_element_manager: ElementManager):
    ...
    # 获取梁单元
    beam_elements = configured_element_manager.get_elements_by_type("forceBeamColumn")
...
def test_get_elements_by_node(configured_element_manager: ElementManager):
    """测试获取包含特定节点的单元"""
    # 获取包含节点2的单元
    elements = configured_element_manager.get_elements_by_nodes(2)
    assert len(elements) == 2  # 两个单元都包含节点2


def test_clear(configured_element_manager: ElementManager):
    """测试清除单元数据"""
    assert len(configured_element_manager.get_elements()) == 2
    configured_element_manager.clear()
    assert len(configured_element_manager.get_elements()) == 0
